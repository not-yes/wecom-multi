...
### 性能优化 🚀
- **首次启动速度**: 大幅优化 Windows 平台首次启动速度 (提速 70-80%)
- **路径扫描**: 优先扫描 C 和 D 盘,只在必要时扫描其他驱动器
- **Mutex扫描**: 精确匹配 ObjectTypeIndex 16-20,减少 50% 检查量

### 改进 🚀
- **进度反馈**: 添加 "快速扫描" 和 "扩展扫描" 提示
- **日志优化**: 统一使用 println! 输出,移除调试统计开销
- **用户体验**: 点击启动按钮后更快显示第一个实例

### UI/Frontend 优化 🎨
- **扁平化设计**: 圆角增大到 16px,移除非必要边框和阴影
- **Tabs组件**: 应用类型选择改为 Tabs 切换样式
- **全局样式**: 使用 @layer components 统一管理设计系统
- **文档完善**: 添加完整的 UI 开发规范到 CLAUDE.md

### 技术细节 🔧
- 路径扫描: 二级策略 (C/D盘优先 → 其他盘符后备)
- Mutex扫描: ObjectTypeIndex 范围从 15-25 缩小到 16-20
- 移除生产环境的类型统计 (debug_mode = false)
- 保持向后兼容: ObjectTypeIndex = 17 优先,16-20 后备
